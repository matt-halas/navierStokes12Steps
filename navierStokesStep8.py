...
def step(dx, dy, dt, nu, u, v):
    ...
    vn[1:-1, 1:-1] = v_int \
        ...
        + nu * dt / dx**2 * (v[2:, 1:-1] - 2 * v_int + v[:-2, 1:-1]) \
        + nu * dt / dy**2 * (v[1:-1, 2:] - 2 * v_int + v[1:-1, :-2])
    un[:, (0, -1)] = 1
    un[(0, -1), :] = 1
    vn[:, (0, -1)] = 1
    vn[(0, -1), :] = 1
...
def plotVelocity(i):
    if i >= len(Vel_all):
        ...
        plt.ylabel('y')
        plt.grid()
        plt.contourf(x_loc, y_loc, Vel_all[i], cmap='plasma',
            vmin=1, vmax=np.sqrt(5), levels=100)
        plt.quiver(x_loc[::quiver_step], y_loc[::quiver_step],
...
if __name__ == '__main__':
    ...
    animation_1.save('2DConvDiff.gif')
# %%
